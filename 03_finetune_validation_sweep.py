...
def time_weighted_item_scores(train_df: pd.DataFrame, n_items: int, half_life_days: int) -> np.ndarray:
    ...
    counts = np.bincount(train_df['item_idx'].to_numpy(np.int32), weights=weights, minlength=n_items).astype(np.float32)
...
def load_encoded_data(data_dir: Path, validation: str, valid_fraction: float) -> EncodedData:
    ...
    return EncodedData(train_all=train, train_context=train_context, valid=valid, sample_submission=sample, user2idx=user2idx, item2idx=item2idx, idx2user=idx2user, idx2item=idx2item, n_users=len(all_user_ids), n_items=len(all_item_ids), train_item_indices=train_item_indices, validation_name=validation_name)
...
def train_lightgcn(seed: int, cfg: TrainConfig, X_binary: sparse.csr_matrix, n_users: int, n_items: int, device: torch.device) -> Embeddings:
...
def build_lightgcn_component(embeddings: Sequence[Embeddings], user_indices: np.ndarray, norm: str, rrf_k: float) -> np.ndarray:
    out = None
...
def recall_at_k_blend(embeddings: Sequence[Embeddings], pop_component: np.ndarray, alpha: float, valid_df: pd.DataFrame, X_seen: sparse.csr_matrix, allowed_items: np.ndarray, users: Sequence[int], norm: str, rrf_k: float, k: int, batch_size: int, desc: Optional[str]=None) -> float:
...
def main():
    ...
    for hl in half_lives:
        for alpha in alphas:
            ...
            recall_all = recall_at_k_blend(embeddings=embeddings, pop_component=pop_components[hl], alpha=alpha, valid_df=data.valid, X_seen=X_context, allowed_items=data.train_item_indices, users=all_valid_users, norm=args.blend_norm, rrf_k=args.rrf_k, k=args.k, batch_size=args.eval_batch_size, desc=name)
            recall_target = np.nan
            if target_valid_users is not None and len(target_valid_users):
                recall_target = recall_at_k_blend(embeddings=embeddings, pop_component=pop_components[hl], alpha=alpha, valid_df=data.valid, X_seen=X_context, allowed_items=data.train_item_indices, users=target_valid_users, norm=args.blend_norm, rrf_k=args.rrf_k, k=args.k, batch_size=args.eval_batch_size, desc=name + ':target')
