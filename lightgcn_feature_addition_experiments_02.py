#!/usr/bin/env python3
"""
Evaluate high-priority additions around the current best model:
...
This validation script tests five additions one at a time:
...
Outputs:
    outputs/lightgcn_feature_additions_{run_id}/results.csv
    outputs/lightgcn_feature_additions_{run_id}/results_sorted.csv
    outputs/lightgcn_feature_additions_{run_id}/config.json
    outputs/lightgcn_feature_additions_{run_id}/eda.json
"""
...
def parse_dynamic_alpha_specs(s: str) -> List[List[Tuple[int, float]]]:
    """
    Format:
        "3:0.65,6:0.55,12:0.50,999999:0.45;3:0.75,6:0.65,12:0.55,999999:0.45"

    Each semicolon-separated segment is one candidate rule.
    """
    specs = []
...
def trend_score(train_df: pd.DataFrame, n_items: int, short_hl: int, long_hl: int) -> np.ndarray:
    """
    Relative trend / acceleration score.

    A pure ratio can over-rank very rare items. This score rewards items whose
    short-term weighted count is high relative to their long-term count, but
    keeps a small support term so one-off noise is less dominant.
    """
    ...
    for epoch in tqdm(range(1, cfg.epochs + 1), desc=f"{name} seed={seed}"):
        ...
        for _ in range(steps_per_epoch):
            users, pos, neg = sample_bpr_batch(
                active_users, pos_arrays, pos_sets, n_items, cfg.batch_size, rng
            )
...
class MultiSeedLightGCNScorer:
    """
    Averages seed rankings rather than raw dot products.
    This avoids score-scale differences between independently trained LightGCN seeds.
    """
...
class DynamicAlphaBlendScorer:
    def __init__(
        ...
        pop_scorer,
        hist_len: np.ndarray,
        alpha_bins: Sequence[Tuple[int, float]],
        norm: str = "rrf",
        rrf_k: float = 60.0,
    ):
        self.name = name
    ...
    def score_batch(self, user_indices: np.ndarray) -> np.ndarray:
        ...
        alphas = np.array(
            [alpha_for_history_len(int(self.hist_len[int(u)]), self.alpha_bins) for u in user_indices],
            dtype=np.float32,
        )
...
def build_item_category_codes(item_meta: pd.DataFrame, item2idx: Dict[int, int], n_items: int) -> np.ndarray:
    """
    Prefer main_category. Fall back to categories if needed.
    """
...
def evaluate_and_record(
    ...
    data: EncodedData,
    X_seen: sparse.csr_matrix,
    all_valid_users: np.ndarray,
    target_valid_users: Optional[np.ndarray],
    k: int,
    batch_size: int,
    notes: str = "",
) -> dict:
    ...
    rows.append(row)
    print(
        f"{name:70s} | all={recall_all:.6f}"
        + (f" | target={recall_target:.6f}" if not np.isnan(recall_target) else "")
        + f" | {row['time_sec']:.1f}s"
    )
    return row
...
def main():
    ...
    parser.add_argument("--dynamic_alpha_specs", type=str, default=(
        "3:0.65,6:0.55,12:0.50,999999:0.45;"
        "3:0.75,6:0.65,12:0.55,999999:0.45;"
        "3:0.60,6:0.55,12:0.50,999999:0.40"
    ))
    ...
    # Train full-graph LightGCN seeds.
    ...
    full_multi = MultiSeedLightGCNScorer(
        "LightGCN_full_multiseed_" + "-".join(map(str, seeds)),
        full_embeddings,
        rrf_k=args.rrf_k,
    )

    recent_pop = VectorScorer(
        f"recent_pop_{args.recent_half_life}d",
        time_weighted_item_scores(data.train_context, data.n_items, args.recent_half_life),
    )

    hist_len = np.diff(X_context.indptr)

    # Baselines and current best family.
    ...
    for alpha in alpha_grid:
        scorer = WeightedBlendScorer(
            f"single_LightGCN + recent{args.recent_half_life} alpha={alpha:g}",
            [(full_single, 1.0 - alpha), (recent_pop, alpha)],
    ...
    for spec in dynamic_specs:
        scorer = DynamicAlphaBlendScorer(
            f"dynamic_alpha_single_{spec_to_name(spec)}",
            full_single,
            recent_pop,
            hist_len=hist_len,
            alpha_bins=spec,
    ...
    if len(full_embeddings) > 1:
        ...
        for alpha in alpha_grid:
            scorer = WeightedBlendScorer(
                f"multiseed_LightGCN + recent{args.recent_half_life} alpha={alpha:g}",
                [(full_multi, 1.0 - alpha), (recent_pop, alpha)],
                norm=args.blend_norm,
                rrf_k=args.rrf_k,
            )
        ...
        for spec in dynamic_specs:
            scorer = DynamicAlphaBlendScorer(
                f"dynamic_alpha_multiseed_{spec_to_name(spec)}",
                full_multi,
                recent_pop,
                hist_len=hist_len,
                alpha_bins=spec,
    ...
    # 2. Recent-window LightGCN.
    recent_cfg = LightGCNConfig(
        dim=args.dim,
        layers=args.layers,
        epochs=args.recent_lgcn_epochs,
        batch_size=args.batch_size,
        lr=args.lr,
        reg=args.reg,
    )
    ...
    for window in recent_windows:
        ...
        for seed in recent_lgcn_seeds:
            emb = train_lightgcn(
                name=f"LightGCN_recent{window}d",
                seed=seed,
                cfg=recent_cfg,
                X_binary=X_recent,
                n_users=data.n_users,
        ...
        if len(recent_embeddings) == 1:
            ...
            recent_lgcn = MultiSeedLightGCNScorer(
                f"LightGCN_recent{window}d_multiseed_" + "-".join(map(str, recent_lgcn_seeds)),
                recent_embeddings,
                rrf_k=args.rrf_k,
            )
        ...
        for gamma in extra_weight_grid:
            scorer = WeightedBlendScorer(
                f"single_LightGCN + recent_pop + recentLGCN{window} gamma={gamma:g}",
                [(full_single, 0.5), (recent_pop, 0.5), (recent_lgcn, gamma)],
                norm=args.blend_norm,
                rrf_k=args.rrf_k,
            )
            evaluate_and_record(rows, scorer.name, "addition_recent_window_lgcn", scorer, data, X_context, all_valid_users, target_valid_users, args.k, args.eval_batch_size, "Current best plus recent-window LightGCN.")

            if len(full_embeddings) > 1:
                scorer2 = WeightedBlendScorer(
                    f"multiseed_LightGCN + recent_pop + recentLGCN{window} gamma={gamma:g}",
                    [(full_multi, 0.5), (recent_pop, 0.5), (recent_lgcn, gamma)],
                    norm=args.blend_norm,
                    rrf_k=args.rrf_k,
                )
    ...
    for short_hl, long_hl in trend_specs:
        trend = VectorScorer(
            f"trend_{short_hl}v{long_hl}",
            trend_score(data.train_context, data.n_items, short_hl=short_hl, long_hl=long_hl),
        )
        ...
        for gamma in extra_weight_grid:
            scorer = WeightedBlendScorer(
                f"single_LightGCN + recent_pop + trend{short_hl}v{long_hl} gamma={gamma:g}",
                [(full_single, 0.5), (recent_pop, 0.5), (trend, gamma)],
                norm=args.blend_norm,
                rrf_k=args.rrf_k,
            )
            evaluate_and_record(rows, scorer.name, "addition_trend", scorer, data, X_context, all_valid_users, target_valid_users, args.k, args.eval_batch_size, "Current best plus trend acceleration.")

            if len(full_embeddings) > 1:
                scorer2 = WeightedBlendScorer(
                    f"multiseed_LightGCN + recent_pop + trend{short_hl}v{long_hl} gamma={gamma:g}",
                    [(full_multi, 0.5), (recent_pop, 0.5), (trend, gamma)],
                    norm=args.blend_norm,
                    rrf_k=args.rrf_k,
                )
    ...
    if data.item_meta is not None:
        cat_pop = CategoryConditionedPopularityScorer(
            f"category_conditioned_recent{args.recent_half_life}",
            data.train_context,
            data.item_meta,
            data.item2idx,
            n_items=data.n_items,
            half_life_days=args.recent_half_life,
            global_weight=0.20,
        )
        ...
        for gamma in extra_weight_grid:
            scorer = WeightedBlendScorer(
                f"single_LightGCN + recent_pop + cat_recent gamma={gamma:g}",
                [(full_single, 0.5), (recent_pop, 0.5), (cat_pop, gamma)],
                norm=args.blend_norm,
                rrf_k=args.rrf_k,
            )
            evaluate_and_record(rows, scorer.name, "addition_category_conditioned", scorer, data, X_context, all_valid_users, target_valid_users, args.k, args.eval_batch_size, "Current best plus user-category-conditioned recent popularity.")

            if len(full_embeddings) > 1:
                scorer2 = WeightedBlendScorer(
                    f"multiseed_LightGCN + recent_pop + cat_recent gamma={gamma:g}",
                    [(full_multi, 0.5), (recent_pop, 0.5), (cat_pop, gamma)],
                    norm=args.blend_norm,
                    rrf_k=args.rrf_k,
                )
