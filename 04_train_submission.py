...
def load_data(data_dir: Path) -> EncodedData:
    ...
    return EncodedData(train=train, sample_submission=sample, user2idx=user2idx, item2idx=item2idx, idx2user=idx2user, idx2item=idx2item, n_users=len(all_user_ids), n_items=len(all_item_ids), train_item_indices=train_item_indices)
...
def train_one_lightgcn(seed: int, cfg: TrainConfig, X_binary: sparse.csr_matrix, X_graph: sparse.csr_matrix, n_users: int, n_items: int, device: torch.device) -> TrainedEmbeddings:
...
def make_lightgcn_component(embeddings: Sequence[TrainedEmbeddings], user_indices: np.ndarray, blend_norm: str, rrf_k: float) -> np.ndarray:
    out = None
...
def generate_predictions(data: EncodedData, X_seen: sparse.csr_matrix, embeddings: Sequence[TrainedEmbeddings], pop_score_vectors: Dict[int, np.ndarray], alphas: Sequence[float], blend_norm: str, rrf_k: float, batch_size: int, output_dir: Path, name_prefix: str, dynamic_alpha_bins: Optional[List[Tuple[int, float]]]=None, trend_score_vectors: Optional[Dict[Tuple[int, int], np.ndarray]]=None, trend_gammas: Optional[Sequence[float]]=None, trend_base_alphas: Optional[Sequence[float]]=None) -> List[Path]:
...
def main():
    ...
    output_paths = generate_predictions(data=data, X_seen=X_binary, embeddings=embeddings, pop_score_vectors=pop_score_vectors, alphas=alphas, blend_norm=args.blend_norm, rrf_k=args.rrf_k, batch_size=args.prediction_batch_size, output_dir=out_dir, name_prefix=name_prefix, dynamic_alpha_bins=dynamic_bins, trend_score_vectors=trend_score_vectors, trend_gammas=trend_gammas, trend_base_alphas=trend_base_alphas)
