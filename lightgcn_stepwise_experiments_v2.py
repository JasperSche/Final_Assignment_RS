#!/usr/bin/env python3
"""
Stepwise LightGCN expansion experiments.

Starts from a plain LightGCN baseline and adds one extra signal at a time:
recency, category metadata, itemKNN, sequence transitions, content TF-IDF,
time-decayed LightGCN, popularity-negative LightGCN, and SASRec.

Expected files in --data_dir: train.csv, test.csv, item_meta.csv, sample_submission.csv.
Outputs: outputs/lightgcn_stepwise_{run_id}/results.csv plus config/eda JSON files.
"""
...
def build_item_static(item_meta: pd.DataFrame, all_item_ids: Sequence[int], item2idx: Dict[int, int]) -> pd.DataFrame:
    ...
    base["text_for_content"] = (
        base["title"] + " " + base["main_category"] + " " + base["categories"] + " " +
        base["features"] + " " + base["description"] + " " + base["store"] + " " + base["details"]
    ).str.replace(r"\s+", " ", regex=True).str.strip()
    keep = ["item_idx", "item_id", "main_category_code", "store_code", "has_metadata",
            "average_rating", "rating_number", "price", "text_for_content"]
...
def split_provided_test(train: pd.DataFrame, test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, float]:
    ...
    overlap = (
        ...
        .merge(train[KEY_COLS].drop_duplicates().assign(_in_train=1), on=KEY_COLS, how="left")
...
def load_encoded_data(data_dir: Path, validation: str, valid_fraction: float) -> EncodedData:
    ...
    return EncodedData(train, train_context, valid, item_meta_raw, sample_submission, user2idx, item2idx,
                       idx2user, idx2item, len(all_user_ids), len(all_item_ids), train_item_indices,
                       item_static, validation_name)
...
def build_norm_adj(X_weighted: sparse.csr_matrix, n_users: int, n_items: int, device: torch.device) -> torch.Tensor:
    ...
    vals = vals_raw * np.power(deg[rows] + 1e-8, -0.5) * np.power(deg[cols] + 1e-8, -0.5)
...
def train_lightgcn(cfg: LightGCNConfig, X_binary: sparse.csr_matrix, X_graph: sparse.csr_matrix, n_users: int, n_items: int, device: torch.device) -> LightGCNScorer:
    ...
    for epoch in tqdm(range(1, cfg.epochs + 1), desc=cfg.name):
        ...
        for _ in range(steps_per_epoch):
            ...
            users_t = torch.LongTensor(users).to(device); pos_t = torch.LongTensor(pos).to(device); neg_t = torch.LongTensor(neg).to(device)
            user_e, item_e = model.propagate(norm_adj)
            u_e, p_e, n_e = user_e[users_t], item_e[pos_t], item_e[neg_t]
            pos_scores = (u_e * p_e).sum(dim=1); neg_scores = (u_e * n_e).sum(dim=1)
...
class VectorScorer:
    def __init__(self, name: str, item_scores: np.ndarray):
        self.name = name; self.item_scores = item_scores.astype(np.float32)
    def score_batch(self, user_indices: np.ndarray) -> np.ndarray:
...
class SparseLinearScorer:
    def __init__(self, name: str, X_user_item: sparse.csr_matrix, W_item_item: sparse.csr_matrix):
        self.name = name; self.X = X_user_item.tocsr(); self.W = W_item_item.tocsr()
    def score_batch(self, user_indices: np.ndarray) -> np.ndarray:
...
class CategoryPopularityScorer:
    def __init__(self, name: str, train_df: pd.DataFrame, item_static: pd.DataFrame, n_items: int, half_life_days: int = 365):
        self.name = name; self.n_items = n_items
        ...
        for c in np.unique(self.item_cat):
            if c < 0: continue
            s = np.zeros(n_items, dtype=np.float32); mask = self.item_cat == c; s[mask] = self.global_scores[mask]
            self.cat_scores[int(c)] = s
    def _dominant_user_category(self, train_df):
        tmp = train_df[["user_idx", "item_idx"]].copy(); tmp["cat"] = self.item_cat[tmp["item_idx"].values]; tmp = tmp[tmp["cat"] >= 0]
        if len(tmp) == 0: return {}
...
class ContentTfidfScorer:
    def __init__(self, name: str, X_user_item: sparse.csr_matrix, item_static: pd.DataFrame, n_items: int, quick: bool = False):
        self.name = name; self.X = X_user_item.tocsr(); self.n_items = n_items
        text = item_static.sort_values("item_idx")["text_for_content"].fillna("").astype(str).values
        max_word = 80_000 if quick else 200_000; max_char = 40_000 if quick else 100_000
        self.vectorizer = FeatureUnion([
            ("word", TfidfVectorizer(analyzer="word", ngram_range=(1, 2), min_df=2, max_features=max_word, sublinear_tf=True, dtype=np.float32)),
            ("char", TfidfVectorizer(analyzer="char_wb", ngram_range=(3, 5), min_df=2, max_features=max_char, sublinear_tf=True, dtype=np.float32)),
        ])
...
def sanitize_scores(scores: np.ndarray, bad_value: float = 0.0) -> np.ndarray:
    """Make model scores safe for blending/evaluation.

    This is especially important for Transformer/SASRec experiments in quick runs:
    if one component emits NaNs, a tiny blend weight can otherwise poison the
    whole LightGCN blend and produce Recall@10 = 0.
    """
    scores = np.asarray(scores, dtype=np.float32)
    return np.nan_to_num(scores, nan=bad_value, posinf=1e6, neginf=-1e6).astype(np.float32, copy=False)
...
def normalize_score_matrix(scores: np.ndarray, method: str = "zscore") -> np.ndarray:
    ...
    if method == "zscore":
        ...
        # If a row is constant, return all zeros instead of NaNs/huge noise.
        return ((scores - mu) / (sigma + 1e-6)).astype(np.float32)
    if method == "minmax":
...
class BlendScorer:
    def __init__(self, name: str, base, addon, alpha: float, norm: str = "zscore"):
        self.name = name; self.base = base; self.addon = addon; self.alpha = float(alpha); self.norm = norm
    def score_batch(self, user_indices: np.ndarray) -> np.ndarray:
...
class WeightedBlendScorer:
    def __init__(self, name: str, components: List[Tuple[object, float]], norm: str = "zscore"):
        self.name = name; self.components = components; self.norm = norm
    def score_batch(self, user_indices: np.ndarray) -> np.ndarray:
        out = None
...
def time_decay_weights(train_df: pd.DataFrame, half_life_days: int) -> np.ndarray:
    max_ts = train_df["timestamp"].max(); age_days = (max_ts - train_df["timestamp"].to_numpy()) / MS_PER_DAY
...
def time_weighted_item_scores(train_df: pd.DataFrame, n_items: int, half_life_days: Optional[int] = None, since_days: Optional[int] = None) -> np.ndarray:
    ...
    if len(df) == 0: return np.zeros(n_items, dtype=np.float32)
...
def topk_sparse_rows(M: sparse.csr_matrix, k: int) -> sparse.csr_matrix:
    M = M.tocsr(); rows=[]; cols=[]; data=[]
    for i in tqdm(range(M.shape[0]), desc=f"topk rows k={k}", leave=False):
        start, end = M.indptr[i], M.indptr[i+1]; c = M.indices[start:end]; d = M.data[start:end]
        if len(d) == 0: continue
        if len(d) > k:
            idx = np.argpartition(-d, k-1)[:k]; c = c[idx]; d = d[idx]
        order = np.argsort(-d); c = c[order]; d = d[order]
        rows.extend([i]*len(c)); cols.extend(c.tolist()); data.extend(d.astype(np.float32).tolist())
    out = sparse.csr_matrix((data, (rows, cols)), shape=M.shape, dtype=np.float32); out.eliminate_zeros(); return out


def build_itemknn_cosine(X: sparse.csr_matrix, topk: int = 300) -> sparse.csr_matrix:
    print(f"Building itemKNN cosine topk={topk}")
    Y = normalize(X.T, norm="l2", axis=1); W = Y @ Y.T
    W = W.tolil(); W.setdiag(0); W = W.tocsr(); W.eliminate_zeros(); return topk_sparse_rows(W, topk)
...
def build_sequence_transition_matrix(train_df: pd.DataFrame, n_items: int, topk: int = 300, max_gap_days: Optional[int] = None) -> sparse.csr_matrix:
    ...
    for _, g in tqdm(train_df.sort_values(["user_idx", "timestamp"]).groupby("user_idx"), desc="user sequences", leave=False):
        items = g["item_idx"].to_numpy(np.int32); ts = g["timestamp"].to_numpy(np.int64)
        ...
        for a in range(len(items)-1):
            ...
            if max_gap_days is not None:
                ...
                if gap_days < 0 or gap_days > max_gap_days: continue
            rows.append(i); cols.append(j); data.append(0.5 + (a+1)/max(1, len(items)-1))
    if not rows: return sparse.csr_matrix((n_items, n_items), dtype=np.float32)
    W = sparse.csr_matrix((np.array(data, dtype=np.float32), (rows, cols)), shape=(n_items, n_items), dtype=np.float32)
    W.sum_duplicates(); W = normalize(W, norm="l1", axis=1); return topk_sparse_rows(W, topk)
...
def build_recent_history_matrix(train_df: pd.DataFrame, n_users: int, n_items: int, max_len: int = 5) -> sparse.csr_matrix:
    ...
    for u, g in train_df.sort_values(["user_idx", "timestamp"]).groupby("user_idx"):
        ...
        rows.extend([int(u)]*len(items)); cols.extend(items.tolist()); data.extend(weights.tolist())
    H = sparse.csr_matrix((data, (rows, cols)), shape=(n_users, n_items), dtype=np.float32); H.sum_duplicates(); return H


class SASRecDataset(Dataset):
    def __init__(self, train_df: pd.DataFrame, max_len: int, n_items: int, seed: int = 42, max_examples_per_user: Optional[int] = None):
        self.max_len=max_len; self.n_items=n_items; self.rng=np.random.default_rng(seed)
        ...
        for u, arr in self.seqs.items():
            ...
            if max_examples_per_user is not None and len(positions) > max_examples_per_user: positions = positions[-max_examples_per_user:]
            for t in positions: self.examples.append((int(u), int(t)))
    def __len__(self): return len(self.examples)
    def __getitem__(self, idx):
        u, t = self.examples[idx]; arr = self.seqs[u]; prefix = arr[max(0, t-self.max_len):t]; target = int(arr[t])
        seq = np.zeros(self.max_len, dtype=np.int64); seq[-len(prefix):] = prefix + 1
        neg = self.rng.integers(0, self.n_items); seen = self.user_seen[u]; tries=0
        while int(neg) in seen and tries < 100:
            neg = self.rng.integers(0, self.n_items); tries += 1
...
class SASRecNet(nn.Module):
    def __init__(self, n_items: int, max_len: int = 30, hidden_dim: int = 128, n_layers: int = 2, n_heads: int = 2, dropout: float = 0.25):
        ...
        self.item_emb=nn.Embedding(n_items+1, hidden_dim, padding_idx=0); self.pos_emb=nn.Embedding(max_len, hidden_dim)
        ...
        self.encoder=nn.TransformerEncoder(enc_layer, num_layers=n_layers); self.dropout=nn.Dropout(dropout); self.norm=nn.LayerNorm(hidden_dim)
        nn.init.normal_(self.item_emb.weight, std=0.02); nn.init.normal_(self.pos_emb.weight, std=0.02)
        # nn.init overwrites padding_idx's default zero vector; restore it.
        with torch.no_grad():
            self.item_emb.weight[0].fill_(0.0)
    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        B, L = seq.shape; positions = torch.arange(L, device=seq.device).unsqueeze(0).expand(B, L)
        x = self.dropout(self.item_emb(seq) + self.pos_emb(positions))
        causal_mask = torch.triu(torch.ones(L, L, device=seq.device), diagonal=1).bool(); padding_mask = seq.eq(0)

        # TransformerEncoder can produce NaNs for fully padded rows because all
        # attention keys are masked. Avoid feeding fully padded rows through it.
...
class SASRecScorer:
    def __init__(self, name: str, model: SASRecNet, train_df: pd.DataFrame, n_items: int, max_len: int, device: torch.device):
        self.name=name; self.model=model.eval(); self.n_items=n_items; self.max_len=max_len; self.device=device
    ...
    def _make_seq_batch(self, user_indices):
        ...
        for r, u in enumerate(user_indices):
            ...
            if len(hist): seqs[r, -len(hist):] = hist + 1
        return seqs
    def score_batch(self, user_indices):
        outs=[]; bs=512
        with torch.no_grad():
            for st in range(0, len(user_indices), bs):
                batch = user_indices[st:st+bs]; seq = torch.LongTensor(self._make_seq_batch(batch)).to(self.device)
                h = self.model(seq); scores = h @ self.model.item_emb.weight[1:].T
...
def train_sasrec(name: str, train_df: pd.DataFrame, n_items: int, device: torch.device, epochs: int, hidden_dim: int, layers: int, heads: int, max_len: int, dropout: float, batch_size: int, seed: int):
    ...
    for epoch in tqdm(range(1, epochs+1), desc=name):
        ...
        for seq, pos, neg in loader:
            ...
            h = model(seq); pos_e = model.item_emb(pos); neg_e = model.item_emb(neg)
            ...
            if not torch.isfinite(loss):
                ...
                continue
            opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0); opt.step(); total += float(loss.detach().cpu())
...
def estimate_recall_upper_bound(valid_df, X_seen, allowed_items, users, k):
    allowed=set(int(x) for x in allowed_items); truth=build_truth(valid_df); vals=[]
    for u in users:
        t = truth.get(int(u), set())
        if not t: continue
        seen=set(int(x) for x in X_seen[int(u)].indices); reachable=[it for it in t if it in allowed and it not in seen]
...
def recall_at_k(scorer, valid_df, X_seen, allowed_items, users, k=10, batch_size=256, desc=None):
    truth = build_truth(valid_df); users = np.array([int(u) for u in users if int(u) in truth], dtype=np.int32)
    if len(users) == 0: return float("nan")
    allowed_items = np.asarray(allowed_items, dtype=np.int32); recalls=[]
    iterator = range(0, len(users), batch_size)
    if desc: iterator = tqdm(iterator, desc=desc, leave=False)
    for st in iterator:
        batch_users = users[st:st+batch_size]
        scores = sanitize_scores(np.asarray(scorer.score_batch(batch_users), dtype=np.float32), bad_value=-1e6)
        scores_allowed = scores[:, allowed_items].copy()
        for r, u in enumerate(batch_users):
            seen = X_seen[int(u)].indices
            if len(seen): scores_allowed[r, np.isin(allowed_items, seen, assume_unique=False)] = -np.inf
            row = scores_allowed[r]; finite = np.isfinite(row)
            if not np.any(finite): recalls.append(0.0); continue
            kk = min(k, int(finite.sum())); pos = np.where(finite)[0]; vals = row[pos]
            if len(pos) > kk:
                part = np.argpartition(-vals, kk-1)[:kk]; top_items = allowed_items[pos[part][np.argsort(-vals[part])]]
            else:
                top_items = allowed_items[pos[np.argsort(-vals)]]
            t = truth.get(int(u), set()); recalls.append(len(set(int(x) for x in top_items[:k]) & t) / min(k, len(t)))
...
def evaluate_and_record(rows, name, group, scorer, data, X_seen, allowed_items, all_users, target_users, k, batch_size, notes=""):
    t0=time.time(); r_all = recall_at_k(scorer, data.valid, X_seen, allowed_items, all_users, k, batch_size, desc=name)
    r_target = np.nan
...
def parse_alpha_grid(s): return [float(x.strip()) for x in s.split(",") if x.strip()]
...
def evaluate_blend_grid(rows, base, addon, addon_name, data, X_seen, allowed_items, all_users, target_users, alpha_grid, k, batch_size, norm):
    best_alpha=None; best=-1.0
    for alpha in alpha_grid:
        name=f"LightGCN + {addon_name} alpha={alpha:g}"; scorer=BlendScorer(name, base, addon, alpha, norm)
        ...
        if np.isnan(metric): metric = row["recall_at_10_all_valid_users"]
        if metric > best: best=float(metric); best_alpha=float(alpha)
    return best_alpha, best


def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--data_dir", default="data"); ap.add_argument("--output_root", default="outputs"); ap.add_argument("--run_id", default=None)
    ap.add_argument("--validation", default="provided_test", choices=["provided_test", "temporal"]); ap.add_argument("--valid_fraction", type=float, default=0.15)
    ap.add_argument("--seed", type=int, default=42); ap.add_argument("--device", default="auto")
    ap.add_argument("--dim", type=int, default=128); ap.add_argument("--layers", type=int, default=3); ap.add_argument("--epochs", type=int, default=120)
    ap.add_argument("--batch_size", type=int, default=4096); ap.add_argument("--lr", type=float, default=2e-3); ap.add_argument("--reg", type=float, default=1e-4)
    ap.add_argument("--time_decay_half_life_days", type=int, default=365); ap.add_argument("--itemknn_topk", type=int, default=300); ap.add_argument("--seq_topk", type=int, default=300); ap.add_argument("--seq_recent_len", type=int, default=5)
    ap.add_argument("--include_sasrec", action="store_true", default=True); ap.add_argument("--skip_sasrec", action="store_true")
    ap.add_argument("--sasrec_epochs", type=int, default=80); ap.add_argument("--sasrec_hidden_dim", type=int, default=128); ap.add_argument("--sasrec_layers", type=int, default=2); ap.add_argument("--sasrec_heads", type=int, default=2); ap.add_argument("--sasrec_max_len", type=int, default=30); ap.add_argument("--sasrec_dropout", type=float, default=0.25); ap.add_argument("--sasrec_batch_size", type=int, default=512)
    ap.add_argument("--alpha_grid", default="0.02,0.05,0.1,0.2,0.35,0.5"); ap.add_argument("--blend_norm", default="zscore", choices=["zscore", "minmax", "rrf", "none"])
    ap.add_argument("--k", type=int, default=10); ap.add_argument("--eval_batch_size", type=int, default=256); ap.add_argument("--max_eval_users", type=int, default=None); ap.add_argument("--quick", action="store_true")
    args=ap.parse_args()
    if args.skip_sasrec: args.include_sasrec=False
    if args.quick:
        args.epochs=min(args.epochs,10); args.sasrec_epochs=min(args.sasrec_epochs,10); args.max_eval_users=args.max_eval_users or 1000; args.itemknn_topk=min(args.itemknn_topk,100); args.seq_topk=min(args.seq_topk,100)
        print("Running in --quick mode.")
    seed_everything(args.seed); device=torch.device("cuda" if args.device=="auto" and torch.cuda.is_available() else ("cpu" if args.device=="auto" else args.device))
    out_dir=Path(args.output_root)/f"lightgcn_stepwise_{args.run_id or now_run_id()}"; out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir/"config.json","w") as f: json.dump(vars(args), f, indent=2)
    print("Output dir:", out_dir.resolve()); print("Device:", device)
    data=load_encoded_data(Path(args.data_dir), args.validation, args.valid_fraction)
    X_binary=make_interaction_matrix(data.train_context, data.n_users, data.n_items, binary_after_sum=True); allowed_items=data.train_item_indices
    ...
    if args.max_eval_users is not None and len(all_users)>args.max_eval_users:
        rng=np.random.default_rng(args.seed); all_users=np.array(sorted(rng.choice(all_users, size=args.max_eval_users, replace=False)), dtype=np.int32)
    target_users=None
    ...
    with open(out_dir/"eda.json","w") as f: json.dump(eda, f, indent=2)
    print(json.dumps(eda, indent=2))
    results=[]; alpha_grid=parse_alpha_grid(args.alpha_grid); best_alphas={}; scorers={}
    ...
    simple=[
        ...
        ("category_pop_365d", CategoryPopularityScorer("category_pop_365d", data.train_context, data.item_static, data.n_items, 365), "Metadata category-aware recent popularity."),
    ]
    for name, scorer, note in simple:
        scorers[name]=scorer; evaluate_and_record(results,name,"standalone_signal",scorer,data,X_binary,allowed_items,all_users,target_users,args.k,args.eval_batch_size,note)
    ...
    W_itemknn=build_itemknn_cosine(X_binary, args.itemknn_topk); itemknn=SparseLinearScorer("itemknn_cosine", X_binary, W_itemknn); scorers["itemknn_cosine"]=itemknn
    ...
    W_seq=build_sequence_transition_matrix(data.train_context,data.n_items,args.seq_topk); H_recent=build_recent_history_matrix(data.train_context,data.n_users,data.n_items,args.seq_recent_len); seq_transition=SparseLinearScorer("sequence_transition",H_recent,W_seq); scorers["sequence_transition"]=seq_transition
    ...
    content=ContentTfidfScorer("content_tfidf",X_binary,data.item_static,data.n_items,quick=args.quick); scorers["content_tfidf"]=content
    ...
    with open(out_dir/"best_alphas.json","w") as f: json.dump(best_alphas,f,indent=2)
    print("\n=== Best results ==="); print(df.head(20).to_string(index=False))
